...
struct PrayerView: View {
    ...
    NavigationStack {
      ...
      .toolbar {
        ToolbarItem(placement: .topBarTrailing) {
          Button { model.location.requestLocation() } label: { Image(systemName: "location") }.accessibilityLabel("تحديث الموقع")
        }
      }
    ...
    ScrollView {
      VStack(spacing: 16) {
        VStack(spacing: 4) {
          ...
          Text(Fmt.gregorian(now, tz: .current, numerals: s.numerals)).font(.arabic(14)).foregroundStyle(.secondary)
  ...
  private func nextCard(_ t: PrayerTimes.DayTimeline, now: Date) -> some View {
    ...
    return VStack(spacing: 6) {
      ...
      Text(Fmt.time(t.next.time, tz: .current, hour12: s.hour12, numerals: s.numerals)).font(.arabic(16, weight: .semibold)).foregroundStyle(.white.opacity(0.95))
    ...
    return VStack(spacing: 0) {
      ForEach(Prayer.allCases, id: \.self) { p in
        ...
        HStack {
          ...
          Spacer()
          Text(Fmt.time(t.times[p], tz: .current, hour12: s.hour12, numerals: s.numerals)).font(.system(size: 17, weight: isCurrent ? .bold : .regular, design: .rounded)).monospacedDigit()
        }
  ...
  private func sunnahRow(_ t: PrayerTimes.DayTimeline) -> some View {
    let s = model.settings
    return HStack {
      VStack(alignment: .leading, spacing: 2) { Text("منتصف الليل").font(.arabic(13)).foregroundStyle(.secondary); Text(Fmt.time(t.sunnah.middleOfNight, tz: .current, hour12: s.hour12, numerals: s.numerals)).font(.arabic(16, weight: .semibold)) }
      Spacer()
      VStack(alignment: .leading, spacing: 2) { Text("الثلث الأخير").font(.arabic(13)).foregroundStyle(.secondary); Text(Fmt.time(t.sunnah.lastThird, tz: .current, hour12: s.hour12, numerals: s.numerals)).font(.arabic(16, weight: .semibold)) }
    }
...
struct MethodPicker: View {
  ...
  var body: some View {
    NavigationStack {
      List {
        Section {
          Toggle(isOn: Binding(get: { model.settings.methodIsAutomatic }, set: { v in model.settings.methodIsAutomatic = v; if v { model.applyAutomaticMethodIfNeeded() } })) {
        ...
        Section("الطرق") {
          ForEach(Methods.order, id: \.self) { id in
            ...
            Button {
              model.settings.methodIsAutomatic = false; model.settings.methodId = id; dismiss()
            } label: {
        ...
        Section("مذهب العصر") {
          Picker("مذهب العصر", selection: Binding(get: { model.settings.madhab }, set: { model.settings.madhab = $0 })) {
