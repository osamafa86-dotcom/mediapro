...
test('معدلات البت لكل قارئ متاحة على الخادم (قائمة مُتحقق منها) وروابط الصوت صحيحة', () => {
  ...
  assert.equal(reciterBitrate('ar.abdurrahmaansudais'), 64); assert.equal(reciterBitrate('ar.ibrahimakhbar'), 32); assert.equal(reciterBitrate('ar.husary'), 128);
  assert.equal(reciterBitrate('ar.husary', 5), 64, 'المحاولات بعد آخر بديل تثبت عليه');
